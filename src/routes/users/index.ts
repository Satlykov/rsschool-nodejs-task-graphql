...
const plugin: FastifyPluginAsyncJsonSchemaToTs = async (
  ...
  fastify.delete(
    ...
    async function (request, reply): Promise<UserEntity> {
      ...
      try {
        ...
        const userToDeleteProfile = await fastify.db.profiles.findOne({
          key: 'userId',
        ...
        const userToDeletePosts = await fastify.db.posts.findMany({
          key: 'userId',
          equals: userToDelete.id,
        });
  ...
  fastify.post(
    ...
    async function (request, reply): Promise<UserEntity> {
      ...
      const userFollower = await fastify.db.users.findOne({
        key: 'id',
      ...
      const userIndex = userSubscriber.subscribedToUserIds.findIndex(
      ...
      const subscriberSubscribedToIds = [
        ...userSubscriber.subscribedToUserIds,
        userFollower.id,
      ];
      const followerSubscribedToUserIds = [
        ...userFollower.subscribedToUserIds,
        userSubscriber.id,
      ];
      const userToUpdate = await fastify.db.users.change(request.params.id, {
        subscribedToUserIds: subscriberSubscribedToIds,
      });
      await fastify.db.users.change(request.body.userId, {
        subscribedToUserIds: followerSubscribedToUserIds,
      });
      return userToUpdate;
    }
  );
  ...
  fastify.post(
    ...
    async function (request, reply): Promise<UserEntity> {
      ...
      const userSubscriberIndex = userSubscriber.subscribedToUserIds.findIndex(
        (user) => user === request.body.userId
      );
      const userFollowerIndex = userFollower.subscribedToUserIds.findIndex(
      ...
      const userToUpdate = await fastify.db.users.change(request.params.id, {
        subscribedToUserIds: userSubscriber.subscribedToUserIds.filter(
          (user) => user != request.body.userId
        ),
      });

      await fastify.db.users.change(request.body.userId, {
        subscribedToUserIds: userFollower.subscribedToUserIds.filter(
          (user) => user != request.params.id
        ),
      });
  ...
  fastify.patch(
    ...
    async function (request, reply): Promise<UserEntity> {
      ...
      const userToChange = await fastify.db.users.findOne({
        key: 'id',
        equals: request.params.id,
      });
